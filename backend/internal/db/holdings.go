...
// GetByID returns a single holding (with its snapshot_id) by id.
// ErrNotFound if missing.
func (r *HoldingsRepo) GetByID(ctx context.Context, id uuid.UUID) (domain.Holding, error) {
...
func (r *HoldingsRepo) Update(ctx context.Context, id uuid.UUID, w HoldingWrite) (domain.Holding, error) {
	row := r.pool.QueryRow(ctx, `
		UPDATE holdings
		...
		w.CategoryID, w.Name, w.Detail, w.ValueIdr, w.IsLiability, w.Gram, w.Qty, w.Brand, w.UsdValue, w.Currency, id)
...
// Delete removes a holding by id. ErrNotFound if it didn't exist.
func (r *HoldingsRepo) Delete(ctx context.Context, id uuid.UUID) error {
