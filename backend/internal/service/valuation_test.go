...
func TestComputeHoldingValue(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:        "us_etf priced from usd_idr",
			categoryKey: "us_etf",
			input:       HoldingInput{UsdValue: 1000},
			rate:        rate,
			wantValue:   round64(1000 * (15800.0 / 1000)),
		...
		{
			name:        "us_etf manual value fallback when usd_value<=0",
			categoryKey: "us_etf",
			input:       HoldingInput{UsdValue: 0, ValueIdr: 20000, Detail: "manual etf"},
			rate:        rate,
			wantValue:   20000,
			wantDetail:  "manual etf",
		},
		{
			name:        "us_etf with no rate entry and usd_value>0 errors",
			categoryKey: "us_etf",
			input:       HoldingInput{UsdValue: 500},
			rate:        nil,
			wantErr:     ErrNoRateEntry,
